...
`noop`, never `pass`, and the gate says so out loud (ADR-0049). borromeanRings has governed its
own repository since its second commit — the one that added the gate.
...
held to it by `04_self_description` (ADR-0052): **every** `**N checks**` and `(<word>
gates` in this file is compared with the registry, in every generated block, so a stated
count here is a checked claim rather than a hand-written one. The prose around them —
...
declaring a task finished on a red tree — on the **fast lane**, so a Stop is released on
a partial verdict and the full suite still gates the merge (ADR-0081) — and the records
